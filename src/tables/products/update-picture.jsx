...
const UpdatePicture = () => {
  ...
  useEffect(() => {
    const getPrevData = () => {
      ...
      try {
        axios
          .post(process.env.REACT_APP_API_URL + "api/getPrevPicture", myObj, {
            withCredentials: true,
          })
          .then((data) => {
            if (data.data.status) {
              setPrevData(data.data.image);
            } else {
          ...
          if (!stop) {
            if (file && newImg !== img) {
              ...
              if (file?.size / (1024 * 1024) < 1.5) {
                ...
                myObj.append("id", prevData.id);
                myObj.append("image", file);
                try {
                  axios
                    .post(
                      process.env.REACT_APP_API_URL + "api/updateImage",
                      myObj,
                      { withCredentials: true }
                    )
                    .then((data) => {
            ...
            <form className="left-side" onSubmit={handleData}>
              <div className="controllers">
                <label className="label-2">
                  <img
                    src={`${process.env.REACT_APP_API_URL}Product/images/${prevData?.image_name}`}
                    alt={"img"}
                  />
                </label>
                <button
                ...
                <Link
                  to={`/table/products/pictures/${prevData?.product_id}`}
                  className="label-2"
                >
